function [varargout]=plotGregsData3
...
M = 25;
x = linspace(0,1,M);
N11D = repmat(N11A,1,M).*exp(-repmat(x,length(N11A),1).^2./2./repmat(sig,1,length(x)).^2) + ...
...
sig = SIG(33:60);
x = linspace(0,1,M);
N12D = repmat(N12A,1,M).*exp(-repmat(x,length(N12A),1).^2./2./repmat(sig,1,length(x)).^2) + ...
...
sig = SIG(76:126);
x = linspace(0,1,M);
N13D = repmat(N13A,1,M).*exp(-repmat(x,length(N13A),1).^2./2./repmat(sig,1,length(x)).^2) + ...
...
sig = SIG(147:327);
x = linspace(0,1,M);
N14D = repmat(N14A,1,M).*exp(-repmat(x,length(N14A),1).^2./2./repmat(sig,1,length(x)).^2) + ...
...
allData = repmat(A,1,M).*exp(-repmat(x,length(A),1).^2./2./repmat(SIG,1,length(x)).^2) + ...
    repmat(B,1,M) + m*A*x;
A2 = data.A2;
B2 = data.B2;
Sig2 = data.Sig2;
...
% figure
% hold on
% mesh(x,t(1:375)+7.695,allData(1:375,:)/max(max(allData(1:375,:))))
% mesh(-x,t(1:375)+7.695,allData(1:375,:)/max(max(allData(1:375,:))))
% shading flat
% view(42,42)
...
for i = 1:5:375
plot3(x,ones(1,25)*t(i)+7.695,allData(i,:),'Color',[191 21 21]/255)
plot3(-x,ones(1,25)*t(i)+7.695,allData(i,:),'Color',[191 21 21]/255)
view(180,0)
zlim([500 5500])
pause(0.05)
end
alt = linspace(180,42,60);
az = linspace(0,42,60);
for i = 1:60
    view([alt(i) az(i)])
    pause(0.03)
...
% figure
% hold on
% mesh(x,t(1:375),totalDorsalAllData(1:375,:)/max(max(totalDorsalAllData(1:375,:))))
% mesh(-x,t(1:375),totalDorsalAllData(1:375,:)/max(max(totalDorsalAllData(1:375,:))))
% shading flat
% view(42,42)
...
% figure
% hold on
% mesh(x,t(1:375),allData(1:375,:)-totalDorsalAllData(1:375,:))
% mesh(-x,t(1:375),allData(1:375,:)-totalDorsalAllData(1:375,:))
% shading flat
% view(42,42)

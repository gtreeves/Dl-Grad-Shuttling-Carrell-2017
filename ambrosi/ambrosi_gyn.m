function [ varargout ] = ambrosi_gyn(Params,varargin)
%ambrosi_gyn Ambrosi's version of the Kanodia model, as replicated by
%Michael O'Connell for the "gynogenetic" phenotype
%
%   [ERROR,PENALTY] = ambrosi_gyn(PARAMS) takes a vector of
...
%
%   [ERROR,PENALTY,PROTEIN,TIME,BETA] = ambrosi_gyn(PARAMS) solves as above, but
...
M = struct('NC10',25,'NC10m',25,'NC11',36,'NC11m',36,'NC12',51,'NC12m',...
    51,'NC13',72,'NC13m',72,'NC14',51);
% Length (L2 - L1) in x
L1 = 0; L2 = 1;
% time spans
...
% tspan = struct('NC10',[1 10000],'NC10m',[1 10000],'NC11',[1 10000],...
%     'NC11m',[1 10000],'NC12',[1 10000],...
%     'NC12m',[1 10000],'NC13',[1 10000],...
%     'NC13m',[1 10000],'NC14',[1 40000]);
clear data
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 2x Dorsal %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    x = linspace_offset(0,1,M.NC10)';
    [An,Am,~,Vn,Vc]=nuclearSize(1,'static',M.NC10,'interphase');
    y0 = [zeros(13,1);zeros(13,1);ones(13,1);
        (1/alpha)*ones(13,1);];
...
%     x = linspace_offset(0,1,M.NC10)';
%     [An,Am,~,Vn,Vc]=nuclearSize(1,'static',M.NC10,'interphase');
%     y0 = [zeros(13,1);zeros(13,1);0.5*ones(13,1);
%         (1/alpha)*ones(13,1); ];
%     
%     nt = 1;
%     for i = 1:9                                                         
...
%% nested functions
    function interphase
        ...
        for n = 1:2:4
            y0((n-1)*M.(nc)+1:n*M.(nc),1)=interp1(linspace_offset(0,1,...
                size(y0_,1)),y0_(:,n+1),linspace_offset(0,1,M.(nc)),'pchip','extrap');
            y0((n)*M.(nc)+1:(n+1)*M.(nc),1)=interp1(linspace_offset(0,1,...
                size(y0_,1)),y0_(:,n+1),linspace_offset(0,1,M.(nc)),'pchip','extrap');
        end
        %%%%%%%%%%%%
        y0(1:M.(nc),1) = zeros(M.(nc),1);
        %%%%%%%%%%%%
    ...
    function mitosis
        ...
        if times && nt <=9
            SOL = ode15s(@mit_fun,tspan.(nc),y0,options);
            t = SOL.x; y = SOL.y;
            [time.(xs{nt}),time.(ts{nt})] = meshgrid(x,t);
            nt=nt+1;
        else
